...
instance EncodeTCM Warning where
  encodeTCM warning = case warning of
    ...
    CoverageIssue name pairs -> kind "CoverageIssue"
      ...
      where
            f (tel, ps) = I2A.NamedClause name True $
...
instance EncodeTCM TypeError where
  encodeTCM err = case err of
    ...
    WrongNumberOfConstructorArguments name expected given ->
      kind "WrongNumberOfConstructorArguments"
        [ "name"      @= name
        , "expected"  @= expected
        , "given"     @= given
        ]
    ...
    ShouldBeEmpty t ps -> do
      ps' <- mapM (prettyPattern 0) ps
      kind "ShouldBeEmpty"
        [ "type"            @= t
        , "patterns"        @= ps'
        ]
...
--
    ...
    DeBruijnIndexOutOfScope i cxt names -> kind "DeBruijnIndexOutOfScope"
      [ "empty"           @= False
      , "index"           @= i
      , "context"         #= inTopContext (encodeTCM cxt')
      ]
      where
...
instance EncodeTCM Call where
  encodeTCM call = S.withContextPrecedence F.TopCtx $ case call of
    ...
    CheckArguments range args t1 _ -> kind "CheckArguments"
      [ "range"       @= range
      , "arguments"   #= mapM f args
      , "type1"       @= t1
      ]
      where   f a = S.withContextPrecedence
                      (F.ArgumentCtx F.PreferParen)
                      (A2C.abstractToConcreteHiding a a)
